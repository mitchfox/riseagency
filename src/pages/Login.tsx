...
const Login = () => {
  ...
  const portalFeatures = [
    {
      ...
      title: t('login.feature_analysis', 'Match Analysis'),
      description: t('login.feature_analysis_desc', 'Every performance broken down with expert tactical insights, key moments, and actionable feedback.')
    },
    {
      image: reportScreenshot,
      title: t('login.feature_reports', 'Performance Reports'),
      description: t('login.feature_reports_desc', 'Comprehensive match reports with ratings, stats, and detailed analysis of your contributions.')
    },
    {
      image: sessionsScreenshot,
      title: t('login.feature_programmes', 'Training Programmes'),
      description: t('login.feature_programmes_desc', 'Personalised sessions and workout programmes designed to develop your game week by week.')
    },
    {
      image: paosScreenshot,
      title: t('login.feature_tracking', 'Development Tracking'),
      description: t('login.feature_tracking_desc', 'Monitor your progress with benchmarks, test results, and development milestones over time.')
    }
  ];
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(t('login.email_not_found', 'Email not found. Please contact support to get access.'));
    } catch (error: any) {
    ...
    <div className="min-h-screen bg-background">
      ...
      >
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center">
          <h1 className="text-4xl md:text-6xl font-bebas uppercase tracking-wider text-foreground mb-4">
            {t('login.title', 'PLAYER PORTAL')}
          </h1>
          <p className="text-muted-foreground text-lg md:text-xl mb-8 max-w-2xl mx-auto">
            {t('login.subtitle', 'Your dedicated space for performance analysis, training programmes, and career development.')}
          </p>
          ...
          <form onSubmit={handleEmailLogin} className="max-w-md mx-auto">
            <div className="flex flex-col sm:flex-row gap-3">
              <Input
                ...
                name="email"
                type="text"
                placeholder={t('login.email_placeholder', 'Enter your login')}
                value={email}
      ...
      <div className="py-8 px-4 bg-secondary/30 border-y border-border/30">
        <p className="text-center text-muted-foreground">
          {t('login.not_player', 'Not a player represented by our agency?')}{" "}
          <span className="text-primary font-medium">{t('login.learn_more', 'Learn more about the RISE Portal below ↓')}</span>
        </p>
      </div>
      ...
      <section className="py-16 md:py-24 px-4 bg-background">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12 md:mb-16">
            <p className="text-primary uppercase tracking-widest text-sm mb-3">{t('login.exclusive', 'Exclusive Access')}</p>
            <h2 className="text-3xl md:text-5xl font-bebas uppercase tracking-wider text-foreground mb-4">
              {t('login.what_players_get', "HERE'S WHAT OUR PLAYERS GET")}
            </h2>
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
              {t('login.features_desc', 'Every player on our agency receives a dedicated portal with tools and resources to accelerate their development.')}
            </p>
          </div>
      ...
      <section className="py-16 px-4 bg-secondary/20 border-t border-border/50">
        <div className="max-w-4xl mx-auto">
          <h3 className="text-2xl md:text-3xl font-bebas uppercase tracking-wider text-foreground text-center mb-10">
            {t('login.plus_more', 'PLUS SO MUCH MORE')}
          </h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            {[
              t('login.extra_highlights', 'Video Highlights'),
              t('login.extra_schedules', 'Weekly Schedules'), 
              t('login.extra_guides', 'Positional Guides'),
              t('login.extra_outreach', 'Club Outreach Updates'),
              t('login.extra_testing', 'Physical Testing'),
              t('login.extra_nutrition', 'Nutrition Plans'),
              t('login.extra_psychology', 'Psychology Sessions'),
              t('login.extra_career', 'Career Planning')
            ].map((item, idx) => (
      ...
      <section className="py-16 px-4 bg-background border-t border-border/50">
        <div className="max-w-3xl mx-auto text-center">
          <h2 className="text-2xl md:text-3xl font-bebas uppercase tracking-wider text-foreground mb-4">
            {t('login.want_access', 'WANT ACCESS?')}
          </h2>
          <p className="text-muted-foreground mb-8">
            {t('login.get_in_touch', 'Get in touch to learn more about joining our player development programme.')}
          </p>
          <Button 
            variant="outline" 
