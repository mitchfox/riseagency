...
export const MatchClipPlayer = ({ analysisId, playerName, opponent, onClose }: MatchClipPlayerProps) => {
  ...
  const goToClip = useCallback((index: number) => {
    ...
    setElements([]); // Clear temp annotations on clip change
    setSelectedId(null);
  }, []);
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === 'd' || e.key === 'D') setDrawingMode(prev => !prev);
    };
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, [currentIndex, clips.length, goToClip, togglePlay, onClose]);
  ...
  return (
    <div ref={containerRef} className="fixed inset-0 z-50 bg-[#0a0c10] flex flex-col">
        ...
        <div className="flex items-center gap-1">
          <Button
            ...
            onClick={() => {
              ...
              if (drawingMode) { setElements([]); setSelectedId(null); }
            }}
      ...
      {/* Main area: toolbar | video + canvas | info panel */}
      <div className="flex-1 flex min-h-0">
        ...
        {/* Centre: video + SVG annotation overlay */}
        <div className="flex-1 relative flex items-center justify-center bg-[#0a0c10] min-h-0">
          {currentClip && (
            <>
              <video
                ref={videoRef}
              ...
              {currentClip.action_score != null && String(currentClip.action_score) !== "" && (
                <div className="absolute top-4 right-4 flex flex-col items-end gap-1.5 z-20">
                  ...
                  <span className="text-white/60 text-[10px] bg-black/50 px-2 py-0.5 rounded backdrop-blur-sm">
              ...
              {drawingMode && (
                <div className="absolute top-3 left-1/2 -translate-x-1/2 bg-[#C6A332]/90 text-white text-[10px] px-3 py-1 rounded-full flex items-center gap-1.5 z-30 backdrop-blur-sm">
                  <Pencil className="w-3 h-3" /> Drawing — annotations clear on clip change
                </div>
              )}
            </>
          )}
      ...
      {/* Bottom bar: current action info */}
      {currentClip && (
        <div className="px-4 py-2 bg-[#12151c] border-t border-white/5 flex items-center gap-3 shrink-0">
          ...
          <span className="text-white/10 text-[10px] ml-auto">
            Space to play/pause · ←→ skip · D to draw · Esc to close
          </span>
        </div>
      )}
    </div>
