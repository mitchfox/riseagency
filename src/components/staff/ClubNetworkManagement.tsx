...
import { Plus, X, Search, Upload, Sparkles, Globe, MapPin, Building2, User, ChevronDown, ChevronUp, Loader2, MoreHorizontal, Download, Wand2, UserSearch, FileText, Filter, SortAsc, Share2, ExternalLink, ArrowUpDown } from 'lucide-react';
...
const ClubNetworkManagement = () => {
  ...
  const getClubLogo = useCallback((clubName: string | null): string | null => {
    ...
    for (const l of clubLogos) {
      ...
      if (normL === norm || (normL.length > 3 && norm.length > 3 && (normL.includes(norm) || norm.includes(normL)))) {
        return l.image_url;
      }
    }
    return null;
  ...
  const handleCsvImport = async () => {
    ...
    try {
      ...
      setShowCsvDialog(false); setCsvText(''); setCsvParsedContacts([]); fetchContacts();
    } catch (err: any) {
  ...
  const filteredContacts = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter(c =>
        ...
        c.country?.toLowerCase().includes(q) || c.position?.toLowerCase().includes(q) || c.email?.toLowerCase().includes(q)
      );
    }
    if (countryFilter !== 'all') result = result.filter(c => c.country?.trim() === countryFilter);
  ...
  // Contact Card - larger, glass effect, club logos
  const ContactCard = ({ contact }: { contact: Contact }) => {
    ...
    return (
      <div
        className="group relative backdrop-blur-xl bg-gradient-to-br from-card/90 via-card/70 to-card/50 border border-white/10 rounded-2xl p-5 hover:border-primary/40 hover:shadow-xl hover:shadow-primary/10 transition-all duration-300 cursor-pointer"
        style={{
          boxShadow: '0 4px 30px rgba(0, 0, 0, 0.1), inset 0 1px 0 rgba(255, 255, 255, 0.05)',
        }}
        onClick={() => openEditDialog(contact)}
      >
        {/* Club strip colour accent */}
        {logo && (
        ...
        <div className="absolute top-3 right-3 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            onClick={(e) => { e.stopPropagation(); handleShareContact(contact); }}
        ...
        <div className="flex items-start gap-3.5 mb-3">
          {contact.image_url ? (
            <img src={contact.image_url} alt={contact.name} className="w-12 h-12 rounded-full object-cover ring-2 ring-white/10 shrink-0" />
          ) : (
            <div className="w-12 h-12 rounded-full bg-primary/15 flex items-center justify-center shrink-0 ring-2 ring-white/10">
          ...
          <div className="min-w-0 flex-1">
            <h3 className="font-semibold text-base leading-tight text-foreground">{contact.name}</h3>
            {contact.position && (
              <p className="text-sm text-muted-foreground mt-0.5">{contact.position}</p>
            )}
          </div>
        </div>

        {/* Club with logo and rating */}
        {contact.club_name && (
          <div className="flex items-center gap-2 mb-2.5">
            ...
            {rating ? (
              <Badge variant="outline" className="text-xs h-5 px-1.5 ml-auto shrink-0 border-primary/40 text-primary font-bold">
                R{rating}
              </Badge>
            ) : (
              <Badge variant="outline" className="text-xs h-5 px-1.5 ml-auto shrink-0 opacity-30">
                ?
              </Badge>
            )}
          </div>
        ...
        {contact.country?.trim() && (
          <div className="flex items-center gap-2 mb-3">
            <img src={getCountryFlagUrl(contact.country.trim())} alt={contact.country.trim()} className="w-5 h-3.5 object-cover rounded-sm shrink-0" />
        ...
        {/* Quick action buttons - WhatsApp primary, no call */}
        <div className="flex items-center gap-2 pt-3 border-t border-white/5">
          {contact.phone && (
            <button
              onClick={(e) => { e.stopPropagation(); openExternalUrl(`https://wa.me/${contact.phone!.replace(/[^0-9]/g, '')}`); }}
          ...
          {contact.email && (
            <button
              onClick={(e) => { e.stopPropagation(); openMailto(contact.email!); }}
  ...
  const GroupHeader = ({ groupName, groupContacts }: { groupName: string; groupContacts: Contact[] }) => {
    const countryProfile = groupBy === 'country' ? getCountryProfile(groupName) : null;
    const rating = groupBy === 'club' ? getClubRating(groupName) : null;
    const logo = groupBy === 'club' ? getClubLogo(groupName) : null;
    ...
    return (
      <div className="mb-3">
        ...
        >
          <div className="flex items-center gap-2 min-w-0">
            {groupBy === 'country' && groupName !== 'Uncategorised' && (
              <img src={getCountryFlagUrl(groupName)} alt={groupName} className="w-6 h-4 object-cover rounded-sm shrink-0" />
            )}
            {groupBy === 'club' && logo && (
              <img src={logo} alt="" className="w-5 h-5 object-contain shrink-0" />
            )}
            ...
            {groupBy === 'club' && rating && (
              <Badge variant="outline" className="text-xs border-primary/40 text-primary font-bold shrink-0">R{rating}</Badge>
            )}
            {groupBy === 'club' && !rating && groupName !== 'Uncategorised' && (
              <Badge variant="outline" className="text-xs opacity-30 shrink-0">?</Badge>
            )}
            <Badge variant="secondary" className="text-xs shrink-0">{groupContacts.length}</Badge>
          </div>
          {collapsedGroups.has(groupName) ? <ChevronDown className="h-4 w-4 text-muted-foreground shrink-0" /> : <ChevronUp className="h-4 w-4 text-muted-foreground shrink-0" />}
          <button
            onClick={(e) => { e.stopPropagation(); openProfileEditor(groupBy as 'country' | 'club' | 'role', groupName); }}
        ...
        {/* Country profile summary with league rules inline */}
        {!collapsedGroups.has(groupName) && groupBy === 'country' && countryProfile && (
          <div className="mt-2 mb-3 px-3 py-2 rounded-lg bg-muted/30 border border-border/40 text-xs text-muted-foreground space-y-1">
            {countryProfile.playing_style && <p><span className="font-medium text-foreground/70">Style:</span> {countryProfile.playing_style}</p>}
            {countryProfile.common_formations && <p><span className="font-medium text-foreground/70">Formations:</span> {countryProfile.common_formations}</p>}
            {countryProfile.key_characteristics && <p><span className="font-medium text-foreground/70">Key traits:</span> {countryProfile.key_characteristics}</p>}
            {countryProfile.league_structure && <p><span className="font-medium text-foreground/70">League rules:</span> {countryProfile.league_structure}</p>}
          </div>
        )}
        {!collapsedGroups.has(groupName) && groupBy === 'club' && (() => {
          const p = getClubProfile(groupName);
          if (!p?.description && !p?.playing_style) return null;
          return (
            <div className="mt-2 mb-3 px-3 py-2 rounded-lg bg-muted/30 border border-border/40 text-xs text-muted-foreground">
        ...
        {!collapsedGroups.has(groupName) && groupBy === 'role' && (() => {
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-5">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="w-56">
                    <DropdownMenuItem onClick={handleAiAutoTag} disabled={aiTagging}>
                      <Wand2 className="h-4 w-4 mr-2" />Auto-Tag (Country/Role)
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={handleAiOrganise} disabled={aiOrganising}>
                      <SortAsc className="h-4 w-4 mr-2" />Organise Fields
                    </DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="w-56">
                    <DropdownMenuItem onClick={() => { setCsvText(''); setCsvParsedContacts([]); setShowCsvDialog(true); }}>
                      <Upload className="h-4 w-4 mr-2" />Import CSV (Paste)
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => fileInputRef.current?.click()}>
                      <Upload className="h-4 w-4 mr-2" />Import CSV (File)
                    </DropdownMenuItem>
                    <DropdownMenuSeparator />
                    <DropdownMenuItem onClick={() => handleCsvExport(contacts)}>
                      <Download className="h-4 w-4 mr-2" />Export All ({contacts.length})
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => handleCsvExport(filteredContacts)}>
                      <Download className="h-4 w-4 mr-2" />Export Filtered ({filteredContacts.length})
                    </DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
            ...
            <div className="flex gap-2 mb-3">
              <div className="relative flex-1">
                ...
                <Input placeholder="Search name, club, country, role, email..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10" />
              </div>
              <Button variant={showFilters ? 'default' : 'outline'} size="icon" onClick={() => setShowFilters(!showFilters)} title="Filters & grouping">
            ...
            {showFilters && (
              <div className="flex flex-col sm:flex-row gap-2 mb-4 p-3 rounded-lg bg-muted/30 border border-border/40 animate-in fade-in-0 slide-in-from-top-2 duration-200">
                <Select value={countryFilter} onValueChange={setCountryFilter}>
                  <SelectTrigger className="w-full sm:w-[160px]"><SelectValue placeholder="All Countries" /></SelectTrigger>
                  <SelectContent>
                ...
                <div className="flex items-center gap-1">
                  ...
                  {(['country', 'club', 'role', 'none'] as GroupBy[]).map(g => (
                    <Button key={g} variant={groupBy === g ? 'default' : 'outline'} size="sm" onClick={() => setGroupBy(g)} className="text-xs h-7 px-2.5">
                ...
                <div className="flex items-center gap-1 ml-auto">
                  ...
                  {(['name', 'club_name', 'country'] as SortField[]).map(f => (
                    <Button key={f} variant={sortField === f ? 'default' : 'ghost'} size="sm" onClick={() => handleSort(f)} className="text-xs h-7">
            ...
            {csvParsedContacts.length === 0 ? (
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  <Button variant="outline" onClick={() => setCsvParsedContacts([])} className="flex-1">Re-parse</Button>
                  <Button onClick={handleCsvImport} disabled={csvProcessing} className="flex-1">
                    {csvProcessing ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : null}Import All
                  </Button>
                </div>
              </div>
            )}
          ...
          <div className="space-y-4">
            {showProfileDialog?.type === 'country' && (
              <>
                ...
                <div><Label>Common Formations</Label><Input value={profileEditData.common_formations || ''} onChange={e => setProfileEditData({ ...profileEditData, common_formations: e.target.value })} placeholder="e.g. 4-3-3, 4-2-3-1" /></div>
                <div><Label>Key Characteristics</Label><Textarea value={profileEditData.key_characteristics || ''} onChange={e => setProfileEditData({ ...profileEditData, key_characteristics: e.target.value })} rows={3} placeholder="What defines football in this country..." /></div>
                <div><Label>League Structure & Rules</Label><Textarea value={profileEditData.league_structure || ''} onChange={e => setProfileEditData({ ...profileEditData, league_structure: e.target.value })} rows={3} placeholder="Division structure, promotion/relegation, foreign player rules..." /></div>
            ...
            {showProfileDialog?.type === 'club' && (
              <>
                <div><Label>Description</Label><Textarea value={profileEditData.description || ''} onChange={e => setProfileEditData({ ...profileEditData, description: e.target.value })} rows={3} placeholder="About this club..." /></div>
