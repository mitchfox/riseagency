  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        // ============= FINAL OUTPUT =============
        // Outside player = BLACK OPAQUE (hide marble behind)
        // Inside player with fluid = TRANSPARENT (reveal marble behind)
        // Inside player without fluid = normal player (opaque)
        if (alpha < 0.1) {
          // Outside player - BLACK OPAQUE to match page background
          gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0);
        } else {
          // Inside player - make transparent where fluid is active
